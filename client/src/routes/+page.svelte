<script lang="ts">
  ...
  import { getVinMessage } from '$lib/api/vin';
...
<div class="flex flex-col items-center justify-center px-4 pt-16 md:pt-24 xl:pt-32 pb-16">
  {#if vinMessage}
    <div
      role="alert"
      class="alert alert-warning mb-4 w-full justify-center text-center md:w-3/4 lg:w-1/2 2xl:w-1/3 gap-2"
    >
      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 stroke-current" fill="none" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
      </svg>
      <span class="whitespace-pre-wrap break-words">{vinMessage}</span>
    </div>
  {/if}
